...
function buildUserIPOBacktest(completed) {
  ...
  const scoreRecord = (record) => {
    ...
    if (favorableIndustryCodes.has(record.stockCode)) score += 3;
    else score -= 1;
    score += record.isAH ? -2 : 2;
    if (Number.isFinite(record.sponsorCount)) {
      if (record.sponsorCount <= 2) score += 2;
      else if (record.sponsorCount >= 4) score -= 1;
    }
    ...
    if (Number.isFinite(record.cornerstonePercent)) {
      ...
      else if (record.cornerstonePercent < 10) score -= 1;
    }
    return score;
  };
...
function buildCurrentIPOStrategy(listing, backtest, modelEstimate) {
  ...
  const verdict =
    score >= 72 && predictionReliable && positiveDownside
      ? "值得打"
      : score >= 56
        ? "谨慎打"
        : "不建议";
