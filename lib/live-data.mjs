...
async function fetchYahooChart(symbol, range = "5y") {
  ...
  return {
    ...
    historyOHLC: history.slice(-1300).map((row) => ({
...
function buildIPOModelValidation(completed) {
  ...
  for (const record of sorted) {
    if (prior.length >= 20) {
      ...
      if (
        estimate.fiveDayHighChange &&
        Number.isFinite(record.fiveDayHighChange)
      ) {
        if (fiveDayHighResiduals.length >= 10) {
          const halfWidth = quantile(fiveDayHighResiduals, 0.9);
          fiveDayHighCalibrated.push({
            covered:
              record.fiveDayHighChange >=
                estimate.fiveDayHighChange.mid - halfWidth &&
              record.fiveDayHighChange <=
                estimate.fiveDayHighChange.mid + halfWidth,
          });
        }
        fiveDayHighResiduals.push(
          Math.abs(
            record.fiveDayHighChange - estimate.fiveDayHighChange.mid,
          ),
        );
      }
    }
  ...
  const fiveDayHighTests = tests.filter(
    ({ estimate, record }) =>
      estimate.fiveDayHighChange &&
      Number.isFinite(record.fiveDayHighChange),
  );
  ...
  const validation = {
    ...
    releaseThresholds: {
      minimumGreyMarketSamples: 60,
      minimumFirstDaySamples: 60,
      minimumFiveDaySamples: 60,
      minimumFiveDayHighSamples: 60,
      minimumIntervalCoverage: 85,
      minimumDirectionAccuracy: 70,
      maximumMeanAbsoluteError: 20,
      maximumCalibrationHalfWidth: 28,
    },
    ...
    fiveDayHigh: {
      sampleCount: fiveDayHighCalibrated.length,
      intervalCoverage: rate(
        fiveDayHighCalibrated.filter((row) => row.covered).length,
        fiveDayHighCalibrated.length,
      ),
      directionAccuracy: rate(
        fiveDayHighTests.filter(({ estimate, record }) =>
          sameDirection(
            record.fiveDayHighChange,
            estimate.fiveDayHighChange.mid,
          ),
        ).length,
        fiveDayHighTests.length,
      ),
      meanAbsoluteError: meanAbsoluteError(
        fiveDayHighTests,
        "fiveDayHighChange",
      ),
      calibrationHalfWidth: quantile(fiveDayHighResiduals, 0.9),
    },
  };
  const thresholds = validation.releaseThresholds;
  const metricReady = (metric, minimumSamples) =>
    metric.sampleCount >= minimumSamples &&
  ...
  validation.releaseReady =
    validation.greyMarket.sampleCount >=
      thresholds.minimumGreyMarketSamples &&
    validation.firstDay.sampleCount >=
      thresholds.minimumFirstDaySamples &&
    validation.fiveDay.sampleCount >= thresholds.minimumFiveDaySamples &&
    validation.fiveDayHigh.sampleCount >=
      thresholds.minimumFiveDayHighSamples &&
    metricReady(validation.greyMarket, thresholds.minimumGreyMarketSamples) &&
    metricReady(validation.firstDay, thresholds.minimumFirstDaySamples) &&
    metricReady(validation.fiveDay, thresholds.minimumFiveDaySamples) &&
    metricReady(validation.fiveDayHigh);
  return validation;
}
...
function calibrateIPOEstimate(estimate, validation) {
  ...
  return {
    ...
    fiveDayHighChange: calibrate(
      estimate.fiveDayHighChange,
      validation.fiveDayHigh,
    ),
  };
}
...
function buildUserIPOBacktest(completed) {
  ...
  const rules = [
    ipoRuleResult(
      ...
      strongIndustryRows,
      baselineWinRate,
      "严格按上市日期只使用此前同行业至少 3 只样本；行业胜率越高，历史里越容易出现更好的首日表现。",
    ),
    ipoRuleResult(
      "few-sponsors",
      "少保荐人",
      knownSponsors.filter((record) => record.sponsorCount <= 2),
      baselineWinRate,
      "少保荐、无华泰的样本在回溯里更容易走出相对更好的首日表现，保留为正向参考。",
    ),
    ipoRuleResult(
      "livermore",
      "利弗莫尔证券参与",
      ...
      baselineWinRate,
      "利弗莫尔参与样本不多，但与更高首日表现同向，保留为正向观察项。",
    ),
    ipoRuleResult(
      "oversubscription",
      "公开认购至少 100 倍",
    ...
    ipoRuleResult(
      ...
      baselineWinRate,
      "无华泰的样本在首日结果上往往更友好，仍然兼作风险与加分并存的观察项。",
    ),
    ipoRuleResult(
      ...
      baselineWinRate,
      "公开认购总额按公开发售手数 × 每手股数 × 发售价 × 认购倍数近似计算；样本里这类标的常出现供需错配后的正反馈，保留为加分项。",
    ),
  ];
  ...
  const scoreRecord = (record) => {
    let score = 0;
    if (record.publicOversubscription >= 1000) score += 4;
    else if (record.publicOversubscription >= 100) score += 3;
    else if (record.publicOversubscription >= 20) score += 1;
    else score -= 1;
    if (favorableIndustryCodes.has(record.stockCode)) score += 5;
    else score -= 1;
    score += record.isAH ? -4 : 2;
    if (Number.isFinite(record.sponsorCount)) {
      if (record.sponsorCount <= 2) score += 4;
      else if (record.sponsorCount >= 4) score -= 3;
    }
    if (record.hasHuatai) score -= 4;
    if (record.hasLivermore) score += 3;
    if (
      record.offerPrice >= highPriceThreshold &&
      record.approxSubscriptionAmount <= lowSubscriptionAmountThreshold
    ) {
    ...
    if (Number.isFinite(record.cornerstonePercent)) {
      if (record.cornerstonePercent >= 30) score += 1;
    }
    return score;
  };
  const reviews = completed.map((record) => {
    const score = scoreRecord(record);
    const verdict = score >= 7 ? "值得打" : score >= 3 ? "谨慎打" : "不建议";
    return {
      ...
      fiveDayHighChange: record.fiveDayHighChange,
    };
  });
  const tiers = [
    {
      id: "strong",
      name: "值得打",
      rows: completed.filter((record) => scoreRecord(record) >= 6),
    },
    {
      id: "watch",
      name: "谨慎打",
    ...
    },
    {
      id: "avoid",
      name: "不建议",
      rows: completed.filter((record) => scoreRecord(record) < 2),
    },
  ].map((tier) => ({
    id: tier.id,
    name: tier.name,
...
function buildCurrentIPOStrategy(listing, backtest, modelEstimate) {
  if (listing.allotmentUrl) {
    return {
      ...
      algorithmVersion: "user-strategy-2026-07-v4",
    };
  }
  ...
  if (industryStat?.sampleCount >= 3) {
    if (industryStat.winRate >= userBacktest.baselineWinRate + 8) {
      score += 10;
      positiveSignals.push(
        `同行业 ${industryStat.sampleCount} 只样本首日上涨率 ${industryStat.winRate.toFixed(0)}%`,
      );
    } else if (industryStat.winRate >= userBacktest.baselineWinRate) {
      score += 6;
      positiveSignals.push(
        `同行业 ${industryStat.sampleCount} 只样本首日上涨率 ${industryStat.winRate.toFixed(0)}%`,
      );
  ...
  if (Number.isFinite(listing.publicOversubscription)) {
    if (listing.publicOversubscription >= 1000) {
      ...
      score += 11;
      positiveSignals.push(`公开认购 ${listing.publicOversubscription.toFixed(0)} 倍`);
    } else if (listing.publicOversubscription >= 20) {
      score += 5;
      neutralSignals.push(`公开认购 ${listing.publicOversubscription.toFixed(0)} 倍`);
    } else {
      score -= 7;
  ...
  if (listing.sponsorNames?.length) {
    ...
    if (listing.sponsorNames.length <= 2) {
      score += 7;
      positiveSignals.push(`${listing.sponsorNames.length} 家保荐人，结构更精简`);
    } else if (listing.sponsorNames.length >= 4) {
      score -= 3;
      neutralSignals.push(`${listing.sponsorNames.length} 家保荐人，结构较多`);
    } else {
      neutralSignals.push(`${listing.sponsorNames.length} 家保荐人`);
    }
  } else {
    neutralSignals.push("保荐人字段尚未完整识别");
  }
  if (listing.underwriterNames?.length) {
    if (listing.underwriterNames.length <= 3) {
      score += 5;
      positiveSignals.push(`${listing.underwriterNames.length} 家承销商，结构更精简`);
    } else if (listing.underwriterNames.length >= 6) {
      score -= 4;
      riskSignals.push(`${listing.underwriterNames.length} 家承销商，结构偏多`);
    } else {
      neutralSignals.push(`${listing.underwriterNames.length} 家承销商`);
    }
  }
  ...
  if (
    ...
    score += 8;
    positiveSignals.push("高定价但公开认购总额偏低");
  }
  if (Number.isFinite(listing.cornerstonePercent)) {
    if (listing.cornerstonePercent >= 30) {
      score += 3;
      positiveSignals.push(`基石比例 ${listing.cornerstonePercent.toFixed(1)}%`);
    } else if (listing.cornerstonePercent >= 20) {
      score += 1;
      neutralSignals.push(`基石比例 ${listing.cornerstonePercent.toFixed(1)}%`);
    }
  }
  ...
  return {
    ...
    action:
      verdict === "值得打"
        ? predictionReliable && positiveDownside
...
function isCompletedHKTradingDate(value, now = new Date()) {
  ...
  return Number(parts.hour) * 60 + Number(parts.minute) >= 16 * 60 + 15;
}
...
async function fetchIPOFifthTradingDay(record) {
  ...
  try {
    ...
    rows = rows.filter((row) => isCompletedHKTradingDate(row.date));
  } catch {
  ...
  const firstFiveSessions = rows.slice(0, 5);
  const highestSession = firstFiveSessions.reduce((best, session) =>
    session.high > best.high ? session : best,
  );
  return {
    ...
    fiveDayHighChange: Number(
      (((highestSession.high - record.offerPrice) / record.offerPrice) * 100).toFixed(2),
    ),
    fiveDayHigh: Number(highestSession.high.toFixed(3)),
    fiveDayHighDate: highestSession.date,
...
async function fetchIPOHistoryData() {
  ...
  // Historical aggregators occasionally omit prospectus facts. Keep verified
  // corrections here so point-in-time audits do not treat known facts as absent.
  const verifiedHistoryOverrides = new Map([
    [
      "02797",
      {
        sponsors: "中泰国际融资有限公司",
        sponsorNames: ["中泰国际融资有限公司"],
        sponsorCount: 1,
        underwriterNames: [
          "中泰国际证券有限公司",
          "建银国际金融有限公司",
        ...
        stabilizingManager: "不适用",
        globalOfferShares: 25_000_000,
        isAH: false,
        reviewNote:
          "当时未保存到完整的申购时点快照，因此没有形成可复核的申购结论；历史涨幅不能倒推成当时已经推荐。",
      },
    ],
  ]);
  const normalized = records.map((record) => ({
    ...
    const override = verifiedHistoryOverrides.get(record.stockCode) || {};
    const corrected = { ...record, ...override };
    const sponsorNames = override.sponsorNames || splitIPOEntities(corrected.sponsors);
    const approxSubscriptionAmount =
      Number.isFinite(corrected.offerPrice) &&
      Number.isFinite(corrected.publicOfferLots) &&
      Number.isFinite(corrected.boardLotShares) &&
      Number.isFinite(corrected.publicOversubscription)
        ? corrected.offerPrice *
          corrected.publicOfferLots *
          corrected.boardLotShares *
          corrected.publicOversubscription
        : null;
    return {
      ...corrected,
      sponsorNames,
      sponsorCount: override.sponsorCount || sponsorNames.length || null,
      hasHuatai: /华泰/.test(corrected.sponsors),
      hasLivermore: /利弗莫尔|Livermore/i.test(corrected.sponsors),
...
export function sanitizePublicData(payload) {
  ...
  return {
    ...
    us: payload.us
      ? {
          ...payload.us,
          stocks: (payload.us.stocks || []).map((stock) => {
          ...
          listings: (payload.hk.listings || []).map((listing) => ({
            ...
            historicalReview: listing.historicalReview
              ? {
                  ...
                  fiveDayHighChange:
                    listing.historicalReview.fiveDayHighChange,
                }
              : null,
            modelEstimate: listing.modelEstimate
              ? {
                  ...
                  fiveDayChange: listing.modelEstimate.publishable
                    ...
                    : null,
                  fiveDayHighChange: listing.modelEstimate.publishable
                    ? listing.modelEstimate.fiveDayHighChange
                    : null,
