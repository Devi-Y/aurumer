...
function buildIPOModelValidation(completed) {
  ...
  const validation = {
    method:
      "按上市日期滚动验证，只使用当时之前至少 20 只新股训练，并只用此前预测误差校准区间",
  ...
  validation.releaseReady =
    ...
    validation.fiveDay.intervalCoverage >= 70 &&
    validation.greyMarket.calibrationHalfWidth <= 60 &&
    validation.firstDay.calibrationHalfWidth <= 60 &&
    validation.fiveDay.calibrationHalfWidth <= 60;
  return validation;
}
...
async function fetchIPOHistoryData() {
  ...
  const completed = normalized.filter((record) =>
    Number.isFinite(record.firstDayChange),
  );
