...
function buildUserIPOBacktest(completed) {
  ...
  const scoreRecord = (record) => {
    ...
    if (favorableIndustryCodes.has(record.stockCode)) score += 4;
    else score -= 1;
    score += record.isAH ? -3 : 2;
    if (Number.isFinite(record.sponsorCount)) {
      if (record.sponsorCount <= 2) score += 3;
      else if (record.sponsorCount >= 4) score -= 2;
    }
    if (record.hasHuatai) score -= 3;
    if (record.hasLivermore) score += 2;
    if (
      record.offerPrice >= highPriceThreshold &&
      record.approxSubscriptionAmount <= lowSubscriptionAmountThreshold
    ) {
  ...
  const reviews = completed.map((record) => {
    ...
    const verdict = score >= 6 ? "值得打" : score >= 2 ? "谨慎打" : "不建议";
    return {
...
function buildCurrentIPOStrategy(listing, backtest, modelEstimate) {
  ...
  if (Number.isFinite(listing.cornerstonePercent)) {
    ...
    } else if (listing.cornerstonePercent < 10) {
      score -= 2;
      riskSignals.push(`基石比例仅 ${listing.cornerstonePercent.toFixed(1)}%`);
    }
  }
