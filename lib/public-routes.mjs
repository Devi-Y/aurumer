...
const CURATED_SMART_MONEY_ROUTES = [
  ["value-partners-classic", "价值伙伴经典基金", "港股"],
  ["fidelity-china-special", "富达中国特殊情况基金", "港股"],
  ["jpm-china-growth", "摩根中国增长与收益", "港股"],
  ["chinaamc-largecap", "华夏大盘精选", "A股"],
  ["fullgoal-tianhui", "富国天惠", "A股"],
  ["xq-herun", "兴全合润", "A股"],
];
...
export function buildPublicRouteSpecs(payload, options = {}) {
  ...
  const specs = [
    ...
    { route: "/us-stocks", hash: "us", title: "美股投资", description: "一句话先看七姐妹和热度前三，再进入价格与风险答案。" },
    { route: "/a-shares", hash: "a-shares", title: "A股收息", description: "一句话先看优等收息、稳健收息与高息待核，再进入股息率、价格和现金流研究观察。" },
    { route: "/gurus", hash: "gurus", title: "聪明人持仓", description: "分港股、美股、A股查看长期高收益代表的公开持仓、Why、How 与跟随边界。" },
    { route: "/gold", hash: "gold", title: "黄金投资", description: "一句话先看黄金现在能不能关注，再看国际金价、上海金、宏观驱动和风险。" },
    { route: "/gold/answer", hash: "gold/answer", title: "黄金投资答案", description: "查看黄金当前结论、国际金与上海金观察低位和风险下沿。" },
    { route: "/gold/price", hash: "gold/price", title: "黄金价格位置", description: "同时查看国际金价、上海金 Au99.99、近期走势和研究参考区间。" },
    { route: "/gold/drivers", hash: "gold/drivers", title: "黄金驱动与风险", description: "查看实际利率、美元、金融条件、黄金持仓和上海金溢价。" },
    { route: "/gold/analysis", hash: "gold/analysis", title: "黄金深度分析", description: "把黄金价格、机会成本、美元流动性、持仓拥挤和风险放在同一页。" },
  ];
  ...
  for (const stock of payload.us?.stocks || []) {
    ...
    specs.push({
      ...
      hash: `stock/${symbol}`,
      title: `${symbol} 美股答案`,
      description: `${symbol}${Number.isFinite(price) ? ` 当前价 $${price.toFixed(2)}` : ""}；查看综合分、排名、观察低位、观察上沿、风险下沿研究参考与风险。`,
    });
  }
  ...
  for (const [id, name, market] of CURATED_SMART_MONEY_ROUTES) {
    specs.push({
      route: `/gurus/${encodeURIComponent(id)}`,
      hash: `investor/${id}`,
      title: `${name} 持仓追踪`,
      description: `${name}（${market}）：查看长期公开业绩依据、最新法定持仓、Why、How 和普通用户参考边界。`,
    });
  }
