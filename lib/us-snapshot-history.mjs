...
function summarizeHKApplicationOptimization(candidates) {
  const ranked = [...(candidates || [])].sort((left, right) => {
    const leftCombined = left.combined || {};
    const rightCombined = right.combined || {};
    const lossDelta =
      (leftCombined.lossCount ?? Infinity) - (rightCombined.lossCount ?? Infinity);
    if (lossDelta) return lossDelta;
    const winDelta =
      (rightCombined.bothPositiveRate ?? -Infinity) -
      (leftCombined.bothPositiveRate ?? -Infinity);
    if (winDelta) return winDelta;
    const sampleDelta =
      (rightCombined.sampleCount ?? 0) - (leftCombined.sampleCount ?? 0);
    if (sampleDelta) return sampleDelta;
    return (
      (leftCombined.upperLossBound95 ?? Infinity) -
      (rightCombined.upperLossBound95 ?? Infinity)
    );
  });
  return {
    bestCandidate: ranked[0]
      ? {
          id: ranked[0].id,
          name: ranked[0].name,
          sampleCount: ranked[0].combined?.sampleCount ?? 0,
          lossCount: ranked[0].combined?.lossCount ?? null,
          bothPositiveRate: ranked[0].combined?.bothPositiveRate ?? null,
          upperLossBound95: ranked[0].combined?.upperLossBound95 ?? null,
          eligibleForReview: Boolean(ranked[0].eligibleForReview),
        }
      : null,
    topCandidates: ranked.slice(0, 3).map((candidate) => ({
      id: candidate.id,
      name: candidate.name,
      sampleCount: candidate.combined?.sampleCount ?? 0,
      lossCount: candidate.combined?.lossCount ?? null,
      bothPositiveRate: candidate.combined?.bothPositiveRate ?? null,
      upperLossBound95: candidate.combined?.upperLossBound95 ?? null,
      eligibleForReview: Boolean(candidate.eligibleForReview),
    })),
  };
}

function summarizeUSStrategyOptimization(candidates) {
  const horizonKey = String(DEFAULT_HOLDING_DAYS[0]);
  const ranked = [...(candidates || [])].sort((left, right) => {
    const leftPrimary = left.horizons?.[horizonKey] || {};
    const rightPrimary = right.horizons?.[horizonKey] || {};
    const returnDelta =
      (rightPrimary.averageNetReturn ?? -Infinity) -
      (leftPrimary.averageNetReturn ?? -Infinity);
    if (returnDelta) return returnDelta;
    const winDelta =
      (rightPrimary.winRate ?? -Infinity) - (leftPrimary.winRate ?? -Infinity);
    if (winDelta) return winDelta;
    const drawdownDelta =
      (rightPrimary.averageMaxDrawdown ?? Infinity) -
      (leftPrimary.averageMaxDrawdown ?? Infinity);
    if (drawdownDelta) return drawdownDelta;
    return (rightPrimary.sampleCount ?? 0) - (leftPrimary.sampleCount ?? 0);
  });
  return {
    primaryHorizon: DEFAULT_HOLDING_DAYS[0],
    bestCandidate: ranked[0]
      ? {
          id: ranked[0].id,
          name: ranked[0].name,
          sampleCount: ranked[0].horizons?.[horizonKey]?.sampleCount ?? 0,
          winRate: ranked[0].horizons?.[horizonKey]?.winRate ?? null,
          averageNetReturn: ranked[0].horizons?.[horizonKey]?.averageNetReturn ?? null,
          averageMaxDrawdown: ranked[0].horizons?.[horizonKey]?.averageMaxDrawdown ?? null,
          worstMaxDrawdown: ranked[0].horizons?.[horizonKey]?.worstMaxDrawdown ?? null,
          pricePlanSampleCount: ranked[0].horizons?.[horizonKey]?.pricePlanSampleCount ?? 0,
          eligibleForReview: Boolean(ranked[0].eligibleForReview),
        }
      : null,
    topCandidates: ranked.slice(0, 3).map((candidate) => ({
      id: candidate.id,
      name: candidate.name,
      sampleCount: candidate.horizons?.[horizonKey]?.sampleCount ?? 0,
      winRate: candidate.horizons?.[horizonKey]?.winRate ?? null,
      averageNetReturn: candidate.horizons?.[horizonKey]?.averageNetReturn ?? null,
      averageMaxDrawdown: candidate.horizons?.[horizonKey]?.averageMaxDrawdown ?? null,
      worstMaxDrawdown: candidate.horizons?.[horizonKey]?.worstMaxDrawdown ?? null,
      pricePlanSampleCount: candidate.horizons?.[horizonKey]?.pricePlanSampleCount ?? 0,
      eligibleForReview: Boolean(candidate.eligibleForReview),
    })),
  };
}

function summarizeUSTechnicalOptimization(candidates) {
  const ranked = [...(candidates || [])].sort((left, right) => {
    const leftValidation = left.validation || {};
    const rightValidation = right.validation || {};
    const returnDelta =
      (rightValidation.averageNetReturn ?? -Infinity) -
      (leftValidation.averageNetReturn ?? -Infinity);
    if (returnDelta) return returnDelta;
    const winDelta =
      (rightValidation.winRate ?? -Infinity) - (leftValidation.winRate ?? -Infinity);
    if (winDelta) return winDelta;
    const factorDelta =
      (rightValidation.profitFactor ?? -Infinity) -
      (leftValidation.profitFactor ?? -Infinity);
    if (factorDelta) return factorDelta;
    return (rightValidation.sampleCount ?? 0) - (leftValidation.sampleCount ?? 0);
  });
  return {
    bestCandidate: ranked[0]
      ? {
          id: ranked[0].id,
          name: ranked[0].name,
          sampleCount: ranked[0].validation?.sampleCount ?? 0,
          winRate: ranked[0].validation?.winRate ?? null,
          averageNetReturn: ranked[0].validation?.averageNetReturn ?? null,
          profitFactor: ranked[0].validation?.profitFactor ?? null,
          equityMaxDrawdown: ranked[0].validation?.equityMaxDrawdown ?? null,
          worstTrade: ranked[0].validation?.worstTrade ?? null,
          eligibleForReview: Boolean(ranked[0].eligibleForReview),
        }
      : null,
    topCandidates: ranked.slice(0, 3).map((candidate) => ({
      id: candidate.id,
      name: candidate.name,
      sampleCount: candidate.validation?.sampleCount ?? 0,
      winRate: candidate.validation?.winRate ?? null,
      averageNetReturn: candidate.validation?.averageNetReturn ?? null,
      profitFactor: candidate.validation?.profitFactor ?? null,
      equityMaxDrawdown: candidate.validation?.equityMaxDrawdown ?? null,
      worstTrade: candidate.validation?.worstTrade ?? null,
      eligibleForReview: Boolean(candidate.eligibleForReview),
    })),
  };
}
...
const HK_APPLICATION_POLICIES = [
  ...
  {
    id: "single_sponsor_small_offer_regime",
    name: "单一保荐 + 小发行规模 + 强市场",
    matches: (row) =>
      !row.isAH &&
      row.sponsorCount === 1 &&
...
function buildHKPointInTimeRows(completed) {
  ...
  for (const record of ordered) {
    // Same-day listings are simultaneous outcomes, not information that was
    // available before another IPO on that date. Excluding them prevents a
    // subtle look-ahead leak in market and industry priors.
    const prior = ordered.filter(
      (candidate) => candidate.listingDate < record.listingDate,
    );
...
export function buildStrategyAudit({ history, hkBacktest, usMarketData, generatedAt }) {
  ...
  const hkApplication = evaluateHKApplicationCandidates(hkBacktest?.completed);
  const usStrategy = evaluateUSStrategyCandidates(history);
  const usTechnical = evaluateUSTechnicalCandidates(usMarketData);
  return {
    version: 1,
    ...
    hkApplication: {
      ...hkApplication,
      optimization: summarizeHKApplicationOptimization(hkApplication.candidates),
    },
    us: {
      ...usStrategy,
      optimization: summarizeUSStrategyOptimization(usStrategy.candidates),
    },
    usTechnical: {
      ...usTechnical,
      optimization: summarizeUSTechnicalOptimization(usTechnical.candidates),
    },
  };
}
