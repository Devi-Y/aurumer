...
function detailView(item) {
  ...
  if (item.market === "hk") {
    ...
    base.facts = [["今日涨跌", formatPercent(raw.changePercent)], ["七日涨跌", formatPercent(raw.weeklyChange)], ["热度", `${Number(raw.heatScore || 0)} 分`], ["营收增长", formatPercent(raw.fund && raw.fund.revenueGrowth)], ["利润率", formatPercent(raw.fund && raw.fund.profitMargin)], ["现金及等价物", raw.fund ? formatLarge(raw.fund.liquidAssets * 1000) : "暂缺"]];
    base.risk = "价格区间是研究参考；高热度不等于适合追高。";
  } else if (item.market === "a") {
    const financials = raw.financials || {};
    base.metrics = [["当前价格", `¥${Number(raw.currentPrice || 0).toFixed(2)}`], ["当前股息率", `${Number(raw.currentDividendYield || 0).toFixed(2)}%`], ["合理买入", raw.buyPrice || "待核验"], ["高安全边际", raw.safeMarginPrice || "待核验"]];
    ...
    base.metrics = [["跟踪价值", `${Number(raw.trackingScore || 0)} 分`], ["披露季度", raw.reportDate || "待核验"], ["披露日期", raw.filingDate || "待核验"], ["组合规模", formatLarge(raw.portfolioValue)]];
    base.holdings = (raw.holdings || []).slice(0, 6).map((holding) => ({ name: holding.ticker, value: `${Number(holding.weight || 0).toFixed(1)}% · ${holding.changeLabel || "变化待核验"}` }));
