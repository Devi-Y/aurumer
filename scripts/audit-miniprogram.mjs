...
for (const [group, count] of [["hk", 3], ["us", 5], ["a", 3]]) {
  assert(miniGuruItems.filter((item) => item.group === group).length === count, `小程序聪明人 ${group} 分组应有 ${count} 个`);
}
...
assert(miniEntryOrder.every((position, index) => position >= 0 && (index === 0 || position > miniEntryOrder[index - 1])), "小程序首页顺序必须是港股、美股、A股、黄金、聪明人持仓");
for (const label of ["值得打", "谨慎打", "不建议", "已结束", "七姐妹", "热度前三", "买入", "等待", "回避", "当前答案", "价格位置", "驱动与风险", "港股 · 3 个", "美股 · 5 个", "A股 · 3 个"]) {
  assert(sectionSource.includes(label), `小程序缺少二级入口：${label}`);
}
for (const label of ["买入参考", "止盈参考", "止损参考", "自由现金流", "公开持仓", "站内完整分析", "下一步怎么做", "WHY · 为什么选它", "HOW · 怎么学"]) {
